...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("logs/fetch_long_history_cubes.log", encoding='utf-8'),
        logging.StreamHandler()
    ]
)

class XueqiuLongHistoryHunter:
    def __init__(self):
        ...
        self.output_file = "data/long_history_cubes.json"
        
        # Criteria
        self.min_followers = 1000
        self.min_total_gain = 20.0
        # 2022-01-01 timestamp in milliseconds
        self.max_created_at = datetime(2022, 1, 1).timestamp() * 1000 
        
        self._init_cookie()

    def _init_cookie(self):
        # Hardcoded cookie from previous context (or load from file if I had one)
    ...
    def scan(self, pages=50):
        ...
        for page in range(1, pages + 1):
            ...
            try:
                ...
                for item in data["list"]:
                    ...
                    if created_at < self.max_created_at and followers >= self.min_followers and total_gain >= self.min_total_gain:
                        logging.info(f"Found candidate: {item['name']} ({item['symbol']}) - Created: {datetime.fromtimestamp(created_at/1000).date()}, Gain: {total_gain}")
        ...
        if all_candidates:
            ...
            logging.info(f"Saved {len(existing)} cubes ({new_count} new) to {self.output_file}")
            print(f"Saved {len(existing)} cubes ({new_count} new) to {self.output_file}")
        else:
            logging.info("No candidates found.")

if __name__ == "__main__":
    hunter = XueqiuLongHistoryHunter()
    hunter.scan(pages=20) # Scan 20 pages
