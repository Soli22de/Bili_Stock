...
def _build_group_ret_v41(panel: pd.DataFrame, trim_q: float = 0.05, hold_step: int = 10) -> pd.DataFrame:
    ...
    for d, day in df.groupby("date"):
        ...
        if regime == "上涨":
            n_target = max(1, int(round(0.2 * n)))
...
def _metrics(group_ret: pd.DataFrame) -> dict:
    ...
    return {"up_top_bottom": up_tb, "calmar": calmar, "max_drawdown": mdd}
...
def main():
    ...
    group_ret = _build_group_ret_v41(panel_liq, trim_q=0.05, hold_step=10)
    group_ret = _apply_up_exposure(group_ret, up_scale=0.5)
    m = _metrics(group_ret)
    group_ret[["date", "Bottom30", "Middle40", "Top30"]].to_csv(
        os.path.join(out_dir, "group_ret_baseline_v4_1_2w_2022_2025.csv"), index=False, encoding="utf-8-sig"
    )
    pd.DataFrame(
        {"metric": ["up_top_bottom", "calmar_ratio", "max_drawdown_ls_curve"], "value": [m["up_top_bottom"], m["calmar"], m["max_drawdown"]]}
    ).to_csv(os.path.join(out_dir, "core_metrics_baseline_v4_1_2022_2025.csv"), index=False, encoding="utf-8-sig")
    with open(os.path.join(out_dir, "baseline_v4_1_micro_tune_report.md"), "w", encoding="utf-8") as f:
        f.write("# baseline_v4.1 微调结果（2022-2025）\n\n")
        f.write(f"- 上涨市 top-bottom: {m['up_top_bottom']:.6f}\n")
        f.write(f"- 整体 calmar: {m['calmar']:.6f}\n")
        f.write(f"- 整体 max_drawdown: {m['max_drawdown']:.4f}\n")
